...
// path for CohortRetentionCard under layerchart 2.x. Fixture injection
// strategy: an E2E_FIXTURES=1 env var on the preview server gates a
// ?__e2e=charts query param in +page.server.ts + +layout.server.ts that
...
import { STUB_RETENTION } from './fixtures/charts-stub';
...
test.describe('charts render non-empty data under layerchart 2.x', () => {
  ...
  test('CohortRetentionCard hydrates without scale.copy crash', async ({ page }) => {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const _fixtures = { STUB_RETENTION };
    ...
    // Heading present = card rendered the non-empty branch.
