...
| 260420-wdf | Day-of-week filter (7 checkboxes, Mon..Sun) applied client-side to KPI tiles + calendar + heatmap + per-item cards; retired Lin/Log toggle (locked to log-linear). Repeater card scope-expanded to full Option-A recomputation (visit_count + cohort_month shift under the "what if" lens) via new lifetime-tx SSR fetch. Retention card keeps caveat (deferred). Cloud-verified SQL-exact across all affected charts. | 2026-04-20 | 03db100, 1c9cf3a, 7027b4b, 545273c | [260420-wdf-dow-filter-retire-lin-log](./quick/260420-wdf-dow-filter-retire-lin-log/) |
